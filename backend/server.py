...
def call_bedrock(conversation: List[Dict], user_message: str) -> str:
    ...
    try:
        ...
        if error_code == 'ValidationException':
            ...
            raise HTTPException(status_code=400, detail="Invalid message format for Bedrock") from e
        elif error_code == 'AccessDeniedException':
            print(f"Bedrock access denied: {e}")
            raise HTTPException(status_code=403, detail="Access denied to Bedrock model") from e
        else:
            print(f"Bedrock error: {e}")
            raise HTTPException(status_code=500, detail=f"Bedrock error: {str(e)}") from e
